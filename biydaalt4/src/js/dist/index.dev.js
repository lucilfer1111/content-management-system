...
function createComponent(componentType) {
  switch (componentType) {
    ...
    case 'text':
    case 'link':
      var componentContent = $('#' + componentType + 'Content').val(); // Perform AJAX request to send data to the backend (add.php)
...
function getComponents() {
  $.ajax({
    ...
    success: function success(response) {
      ...
      $('#components').empty(); // Iterate over the response and append components to the container
...
var editor = new FroalaEditor('#editor', {// Configuration options for the editor
